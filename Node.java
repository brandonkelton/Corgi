...
public class Node {
  ...
  //   count++;
  //   if (kind == "stoFunc") {
  //     funcTable.store( inf, this );
  //   }
  //   System.out.println( this );
  // }

  public Node( String k, String inf ) {
    kind = k; info = inf;
    id = count;
    count++;
  }

  public Node( String k, String inf, Node node ) {
    kind = k; info = inf; first = node;
    id = count;
    count++;
  }

  public Node ( String k, Node node ) {
    kind = k; first = node;
    id = count;
    count++;
  }

  public Node ( String k, Node f, Node s ) {
    id = count;
    count++;
    if (f.kind.equals("param")) {
      kind = "function";
      info = k;
      first = f;
      second = s;
      funcTable.store( info, this );
      System.out.println("Stored function: " + info);
    } else {
      kind = k; first = f; second = s;
    }
  }
